...
namespace Content.Client.Access.UI
{
    ...
    public sealed partial class IdCardConsoleWindow : DefaultWindow
    {
        ...
        private static readonly ProtoId<JobPrototype> DefaultJob = "Contractor"; // Frontier: Passenger<Contractor
            ...
            List<ProtoId<AccessLevelPrototype>> accessLevels)
        {
            ...
            foreach (var job in jobs
                         .Where(job => !job.HideConsoleVisibility)
        ...
        public void UpdateState(IdCardConsoleBoundUserInterfaceState state)
        {
            ...
            ShipSuffixLineEdit.Editable =
                false; // "Make sure you cannot change the suffix at all." - @dvir001, 2023.11.16

            if (interfaceEnabled && state.HasOwnedShuttle)
            {
                var parts = state.TargetShuttleNameParts ?? [null, null];
            ...
            // Update state with access lists, or empty lists if either are null.
            _accessButtons.UpdateState(state.TargetIdAccessList?.ToList() ?? [], state.AllowedModifyAccessList?.ToList() ?? []);
