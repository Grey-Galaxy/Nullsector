...
public sealed class PublicTransitSystem : EntitySystem
{
    ...
    /// <summary>
    /// Hardcoded snippet to intercept FTL events. It catches the transit shuttle and ensures its looking for the "DockTransit" priority dock.
    /// </summary>
    ...
    private void PrintBusSchedule(ProtoId<PublicTransitRoutePrototype> route,
        ...
        ref ExaminedEvent args)
    {
        ...
        if (HasComp<FTLComponent>(grid))
        {
            ...
            message.PushNewline();
            message.AddMarkupPermissive(Loc.GetString("bus-schedule-arrival",
    ...
    private void PrintStationSchedule(ProtoId<PublicTransitRoutePrototype> route,
        EntityUid grid,
        ref ExaminedEvent args)
    {
        ...
        var departureTime = nextBus.Comp.NextTransfer +
            stopDistance * (routeData.Prototype.TravelTime + routeData.Prototype.WaitTime) - _ticker.RoundStartTimeSpan;

        FormattedMessage message = new();
        message.AddMarkupPermissive(Loc.GetString("bus-schedule-next-departure",
            ("bus", Name(nextBus)),
    ...
    /// <summary>
    /// Checks to make sure the grid is on the appropriate play-field, i.e., not in mapping space being worked on.
    /// If so, adds the grid to the list of bus stops, but only if it's not already there
    /// </summary>
    private void OnStationStartup(Entity<StationTransitComponent> ent, ref ComponentStartup args)
    {
        if (Transform(ent).MapID !=
            _ticker.DefaultMap) //best solution i could find because of componentinit/mapinit race conditions
            return;
    ...
    private void OnShuttleArrival(Entity<TransitShuttleComponent> ent, ref FTLCompletedEvent args)
    {
        ...
        while (consoleQuery.MoveNext(out var consoleUid, out _, out var xform))
        {
            ...
            _chat.TrySendInGameICMessage(consoleUid,
                Loc.GetString("public-transit-arrival",
    ...
    /// </summary>
    private bool TryGetNextStop(PublicTransitRoute route,
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var comp, out var shuttle))
        {
            ...
            var hyperspaceTime = MathF.Max(0.0f,
                (float)route.Prototype.TravelTime.TotalSeconds - _shuttles.DefaultStartupTime);
            _shuttles.FTLToDock(uid,
                shuttle,
                nextGrid.Value,
                startupTime: _shuttles.DefaultStartupTime,
                hyperspaceTime: hyperspaceTime,
                priorityTag: comp.DockTag); // TODO: Unhard code the priorityTag as it should be added from the system.
            ...
            while (consoleQuery.MoveNext(out var consoleUid, out _, out var xform))
            {
                ...
                _chat.TrySendInGameICMessage(consoleUid,
                    Loc.GetString("public-transit-departure",
                        ("destination", metadata.EntityName),
    ...
    private void SetupPublicTransit(SectorPublicTransitComponent comp)
    {
        ...
        while (busDepotEnumerator.MoveNext(out var depotStation, out _))
        {
            ...
            foreach (var route in comp.Routes.Values.Where(route => route.GridStops.Count > 0))
            {
                var count = 0;
                // This whole while-loop arrangement is a result of an issue from Frontier, and my attempt
                // to fix it. It seems to deftly dodge a crash, and only tracks the stops. Should cause no performance
                // issues, as it is run once overall! -Z
                while (!route.GridStops.TryAdd(count, depotGrid.Value))
                {
                    count++;
                }
        ...
        foreach (var route in comp.Routes.Values)
        {
            ...
            while (numBuses < neededBuses)
            {
                ...
                if (!_loader.TryLoadGrid(dummyMap,
                ...
                transitComp.ScreenText = Loc.GetString("public-transit-shuttle-screen-text",
                    ("number", route.Prototype.RouteNumber),
                    ("suffix", busSuffix));

                EnsureComp<PreventPilotComponent>(shuttleEnt.Owner);

                var shuttleName = Loc.GetString("public-transit-shuttle-name",
                    ("number", route.Prototype.RouteNumber),
                    ("suffix", busSuffix));
                ...
                {
                    _ = _station.InitializeNewStation(stationProto.Stations[busVessel.ID],
                        [shuttleEnt],
                        shuttleName);
                }
                ...
                _shuttles.FTLToDock(shuttleEnt,
                    shuttleComp,
                    nextGrid,
